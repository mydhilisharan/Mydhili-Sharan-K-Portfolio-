...
const scriptReplacement = `<script>
        // ==========================================
        // SHARAN VAULT — Pure Firebase Storage System
        // ==========================================
        ...
        async function renderGallery() {
            ...
            galleryData.forEach((item, index) => {
                // Safeguard against local file paths that might be cached in older records
                if (item.src && (item.src.startsWith('file://') || item.src.startsWith('C:/'))) {
                    return; // Skip broken local data
                }
                ...
                const cardHTML = \`
                    <div class="gallery-item" onclick="openLightbox(\${index})">
                        ...
                        <div class="delete-btn" onclick="deleteMedia(\${index}, event)">
        ...
        async function loadGalleryData() {
            ...
            try {
                ...
                // Explicit getDoc bypassing cache
                const docRef = window.doc(window.db, "portfolio", "gallery");
                ...
                galleryData = []; // Fallback empty state
            } finally {
                renderGallery();
        ...
        window.onload = () => {
            // Give Firebase a tiny bit of time if initialization is delayed
            setTimeout(() => {
                loadGalleryData();
            }, 500);
        ...
        async function addMedia(event) {
            ...
            try {
                ...
                // Push to gallery and save to Firestore
        ...
        // DELETE MEDIA
        async function deleteMedia(index, event) {
            event.stopPropagation(); // Setup lightbox clicks
            ...
            if (confirm("Permanently delete this from the secure cloud?")) {
                try {
                    ...
                    if (item.src.includes('firebasestorage')) {
                        try {
                            // Extract path from download URL or just rely on Firestore delete
                            // A robust delete would parse the URL, but setting Firestore is primary
                            const fileRef = window.ref(window.storage, item.src);
                            await window.deleteObject(fileRef);
                        } catch(e) {
                            console.warn("Storage item delete skipped or failed", e);
                        }
                    }

                    // Remove from array and update Firestore
...
if (start !== -1 && end !== -1) {
    ...
    console.log("ai-gallery.html successfully updated to pure Firebase logic.");
} else {
