...
const s1Marker = '                let mediaSrc = item.src;';
const s2Marker = '        // PREVIEW FILE BEFORE UPLOAD';

let s1 = html.indexOf(s1Marker);
let s2 = html.indexOf(s2Marker);
...
`;
    let newHTML = html.substring(0, s1 + s1Marker.length) + "\\n" + freshCode + html.substring(s2);
    // Replace DB version globally
    newHTML = newHTML.replace(/VaultOfflineDB", 1/g, 'VaultOfflineDB", 3');
    newHTML = newHTML.replace(/VaultOfflineDB", 2/g, 'VaultOfflineDB", 3');
